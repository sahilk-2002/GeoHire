<template>
  <div class="h-screen w-full overflow-hidden flex flex-col lg:pt-0">
    <header class="w-full bg-background z-40 shrink-0 lg:border-b lg:border-outline-variant/50">
      <div class="flex items-center justify-between px-container-margin lg:px-6 py-sm">
        <div class="flex items-center gap-2 lg:hidden">
          <span class="material-symbols-outlined text-primary">explore</span>
          <span class="font-headline-md text-headline-md font-bold text-primary">GeoHire</span>
        </div>
        <h2 class="hidden lg:block font-headline-sm text-headline-sm text-on-surface">Map View</h2>
        <div class="flex items-center gap-3">
          <router-link to="/upload"
            class="text-primary font-label-md text-label-md hover:opacity-80 transition-opacity">
            Upload Resume
          </router-link>
        </div>
      </div>
      <div class="px-container-margin lg:px-6 pb-3 lg:pb-4 max-w-2xl">
        <div class="relative flex items-center">
          ...
          <input v-model="searchQuery" @keyup.enter="searchLocation"
            class="w-full h-11 pl-10 pr-10 bg-surface-container border border-outline-variant rounded-full text-body-sm focus:ring-2 focus:ring-primary focus:border-transparent outline-none transition-all"
    ...
    <main class="flex-1 relative">
      ...
      <div class="absolute right-4 lg:right-6 top-4 lg:top-6 flex flex-col gap-2 z-30">
      ...
      <!-- Desktop side panel -->
      <div v-if="selectedJob" class="hidden lg:block absolute top-4 left-4 w-80 z-30 max-h-[calc(100%-2rem)] overflow-y-auto">
